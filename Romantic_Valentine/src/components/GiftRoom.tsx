...
import { Mail, Heart, Gift, Flower, Camera } from 'lucide-react';

interface GiftRoomProps {
  onSelectGift: (gift: string) => void;
}

export function GiftRoom({ onSelectGift }: GiftRoomProps) {
  const gifts = [
    {
      id: 'letter',
      title: 'A Love Letter',
    ...
    >
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {gifts.map((gift, index) => (
            <motion.button
              ...
              className="group relative bg-white/40 backdrop-blur-xl rounded-3xl shadow-xl hover:shadow-2xl border border-white/60 p-8 md:p-10 text-left transition-all"
            >
        ...
        >
          ...
          <motion.div
            className="mt-4"
            onClick={() => onSelectGift('bubu')}
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
          >
            <button className="text-5xl hover:cursor-pointer transition-transform">
              🧸🧸
            </button>
